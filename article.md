...
* wepy2(https://github.com/Tencent/wepy)
...
* taro next(https://github.com/NervJS/taro)
...
动态测试demo模拟了实际业务中非常常见的长列表+多tab场景，该demo中存在两份优惠券列表数据，一份为可用券数据，另一份为不可用券数据，其中同一时刻视图中只会渲染展示其中一份数据，可以在上方的操作区模拟对列表数据的各种操作及视图展示切换(切tab)。
...
由于并不是所有框架都能够使用`webpack-bundle-analyzer`得到精确的包体积占用，这里我通过将各框架生成的demo项目体积减去native编写的demo项目体积作为框架的运行时体积。
...
结论分析：wepy2和mpx在框架运行时体积上控制得最好，taro next和kbone由于其动态渲染的特性，在dist中会生成递归渲染模板/组件，所以占用体积较大。
...
结论分析：由于mpvue全量在页面进行渲染，kbone和taro next采用了动态渲染技术，页面渲染耗时较长，其余框架并无太大区别。
...
结论分析：mpx和uniapp在框架内部进行了完善的diff优化，随着数据量的增加，两个框架的新增耗时没有显著上升，新增性能逐渐赶上native；wepy2会在数据变更时对props数据也进行setData，在该场景下造成了大量的无效性能损耗，导致性能表现不佳；kbone和taro next采用了动态渲染方案，每次新增更新时会发送大量描述dom结构的数据，与此同时动态递归渲染的耗时也远大于常规的静态模板，使得这两个框架在所有的更新场景下耗时都远大于其他框架。
...
结论分析：具备模板数据跟踪能力的三个框架mpx，kbone和taro next在后台数据场景耗时并没有增加，wepy2当中diff的精度不足，耗时也没有产生明显变化，其余框架由于每次更新都会对后台数据进行diff，耗时都发生了少量提升。
...
结论分析：可以看出所有使用了原生自定义组件进行组件化实现的框架局部更新耗时都极低，也足以证明小程序原生自定义组件的优秀性和重要性；mpvue由于使用了页面更新，局部更新耗时显著增加；而kbone和taro next则是由于递归动态渲染的性能开销巨大，导致局部更新耗时同样巨大。
...
结论分析：mpx框架成功实现了理论上setData的最优，uniapp由于缺失模板追踪紧随其后，chameleon优于组件每次创建时都会进行一次不必要的setData，产生了大量无效setData调用，但是数据的发送本身经过diff，在数据发送量上表现不错；wepy2的组件会在数据更新时调用setData发送已经更新过的props数据，因此也产生了大量无效调用，且diff精度不足。
...
结论分析：除了kbone和taro next采用动态渲染耗时增加，mpvue使用页面模板渲染性能稍差，其余框架的静态页面渲染表现都和原生差不多。

## 结论

综合上述测试数据，我们得到最终的小程序框架运行时性能排名为：

native > mpx > uniapp > chameleon > wepy2 > mpvue > taro next > kbone
